CREATE   procedure spMigrationAuthTables
AS
...
select ref_code,ref_desc from MIRS_RESTORE.dbo.ref_code_table_dtl
...
(
    ...
    (
        SELECT CASE
               ...
               r.login_end_time + ':00' LoginEndTime FOR JSON PATH, WITHOUT_ARRAY_WRAPPER
    )) AS Sunday,
           JSON_QUERY(
    (
        SELECT CASE
    ...
    WHERE ru.parent_agent_cd = r.parent_agent_cd
          AND ru.remit_user_cd = r.remit_user_cd FOR JSON PATH, WITHOUT_ARRAY_WRAPPER
) AS LoginDaysInfo,
...
(
    SELECT email_id AS Email,
           JSON_QUERY('[' + COALESCE(QUOTENAME(phone_no_1, '"'), '') + IIF(phone_no_1 IS NOT NULL
                                                                           AND phone_no_2 IS NOT NULL, ',', '') + COALESCE(QUOTENAME(phone_no_2, '"'), '') + ']') AS Phones,
           JSON_QUERY('[' + COALESCE(QUOTENAME(fax_no_1, '"'), '') + IIF(fax_no_1 IS NOT NULL
                                                                         AND fax_no_2 IS NOT NULL, ',', '') + COALESCE(QUOTENAME(fax_no_2, '"'), '') + ']') AS Faxes
    FROM mirs_restore.dbo.remit_user ru
    WHERE ru.parent_agent_cd = r.parent_agent_cd
          AND ru.remit_user_cd = r.remit_user_cd FOR JSON PATH, WITHOUT_ARRAY_WRAPPER
) AS ContactInfo,
                ...
                UPPER(TRIM(REPLACE([MIRS_RESTORE].[dbo].[f_RemoveNonASCII](r.[remit_user_cd], 2), ' ', ''))) NormalizedUserName,
...
begin
insert into Auth_M1.dbo.Role(Id, Portal, IsActive, CreatedAt, UpdatedAt, CreatedBy, UpdatedBy, DeletedBy, DeletedAt,
